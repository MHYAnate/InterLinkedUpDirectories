...
const ChatNav: React.FC<ChatProps> = ({
	...
	useEffect(() => {
		...
		const retrievedChatDetailRef = collection(database, `chatContact-${senderId}`);

		const handleDeleteContact = async (data: RetrievedContactValue) => {
			try {
				await deleteDoc(doc(retrievedChatDetailRef, data.docid)).then(() => {
					setIsDelete("");
		...
		const  RenderMyContact: any=()=>{
			...
			return filteredMyContactName.map((contact)=>{
				<div id={contact.contactId} className={styles.contactCover}>
					<MyContactComponent contactNumber={contact.contactNumber} contactImg={contact.contactImg} contactName={contact.contactName} lastMsg={contact.lastMsg} handleDeleteContact={handleDeleteMyContact} contact={contact} handleDeleteRContact={handleDeleteContact} retrievedContact={contact}  />
				</div>
			})
		}
