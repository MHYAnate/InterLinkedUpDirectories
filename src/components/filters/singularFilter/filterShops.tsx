...
export default function ShopsFilter() {
	...
	const filteredShopListstate = ShopData.filter((eachItem) => {
		...
		filteredShopListarea.length > 0
			? filteredShopListarea.filter((eachItem) => {
					...
					return selectComplex !==
						(null || undefined || "" || "Select Market Complex")
						? text.includes(selectComplex.toLowerCase())
						: text;
			  })
			: [];
	...
	const filteredShopTag =
		filteredShopMarket.length > 0
			? filteredShopMarket.filter((eachItem) => {
					...
					return shopTag !== (null || undefined || "" || "Select Tag")
						? text.includes(shopTag.toLowerCase())
						: text;
			  })
			: [];
	...
	const currentPosts = filteredShopAddressList.slice(
		indexOfFirstPost,
		indexOfLastPost
	);
		...
		return currentPosts?.map((shop) => (
			<div key={shop.id}>
				<Shop
					...
					docid={raterDetail?.docid}
					name={raterDetail?.name}
					src={raterDetail?.src}
	...
	const updateShopItemSearchInput = (
		event: React.ChangeEvent<HTMLInputElement>
	) => {
					...
					<div className={styles.selectCover}>
						...
						>
							...
							{selectArea === `${areaValue}` &&
								renderAvailableMarketComplexTag()}
						</select>
					</div>
					<div className={styles.selectCover}>
						<select
							value={
								shopTag !== (undefined || null)
									? shopTag
									: selectCountry
									? ""
									: "select market"
